...
void CTorch::HandleAnimEvent(MonsterEvent_t *pEvent)
{
	...
	case HGRUNT_ALLY_AE_DROP_GUN:
		...
		if ( FBitSet( pev->weapons, TORCH_EAGLE ) )
		{
			...
			GetAttachment( 0, vecGunPos, vecGunAngles );
			DropItem( "ammo_357", vecGunPos, vecGunAngles ); // TODO: change to desert eagle when implemented
		}
		break;
	case HGRUNT_ALLY_AE_RELOAD:
...
void CTorch::GibMonster()
{
	...
	{// throw a gun if the grunt has one
		...
		CBaseEntity *pGun = DropItem( "ammo_357", vecGunPos, vecGunAngles ); // TODO: change to saw when implemented;
		if ( pGun )
		{
			pGun->pev->velocity = Vector (RANDOM_FLOAT(-100,100), RANDOM_FLOAT(-100,100), RANDOM_FLOAT(200,300));
			pGun->pev->avelocity = Vector ( 0, RANDOM_FLOAT( 200, 400 ), 0 );
